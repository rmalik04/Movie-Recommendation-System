            ...
            for i, batch_tuple in enumerate(batch_loader):
                ...
                if do_report_now:
                    ...
                    for key, arr in grad_dict.items():
                        ...
                        cur_norm_str = "grad_wrt_%s %11.10f" % (key, norm)
                        avg_grad_norm_str_list.append(cur_norm_str)
                    avg_grad_norm_str = ' | '.join(avg_grad_norm_str_list)

                    print("epoch %11.3f | loss_total % 11.5f | train_AUC % 11.5f | train_MAE % 11.5f | valid_AUC % 11.5f | valid_MAE % 11.5f | %s" % (
                        epoch, loss if epoch <= 2 else smooth_loss,
                        train_perf_dict['auc'], train_perf_dict['mae'], valid_perf_dict['auc'], valid_perf_dict['mae'],
                        avg_grad_norm_str))
